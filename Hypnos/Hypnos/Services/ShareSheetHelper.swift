...
// macOS has its own share sheet (NSSharingServicePicker), but nothing in the
// macOS UI (Views/Mac/) currently offers a Share button, so this is a known
// gap rather than a port yet — see Hypnos/CLAUDE.md "macOS".
#if !os(tvOS) && !os(macOS)
