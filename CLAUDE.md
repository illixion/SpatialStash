...
- **StereoPump** - `@unchecked Sendable`; owns **all** per-frame GPU work on a dedicated background queue with its own `MTLCommandQueue` — **never** the main thread (an earlier main-actor pump SIGKILLed backboardd via the compositor watchdog). Each tick: pull the newest decoded frame, compute depth **synchronously** for that exact frame (no stale-depth ghosting), warp into two BGRA eye targets, convert each to full-range 420f via `VTPixelTransferSession` (carrying the source's color tags), tag `.leftEye`/`.rightEye`, and enqueue a `CMReadySampleBuffer`. 30 fps cap; bounded eye-buffer pool. Each eye gets its **own** depth attachment (sharing one cross-contaminated the right eye = over-warp). It runs the occlusion-correct depth-displaced mesh warp with the real depth map. (A cheap per-pixel heuristic warp path still exists but is gated out — fake-3D now requires a model; `Pseudo3DStereoEngine.load` bails to the 2D player when none is available.)
- **CoreMLDepthProvider** (conforms to **DepthProvider**) - Optional Core ML monocular depth (Depth Anything V2) driving the mesh warp; `depth(for:) -> MTLTexture?` is synchronous. Model is loaded by name from the managed store (`DepthModelStore`), honoring `AppModel.preferredDepthModelName`. **Fake-3D requires a real model — there is no heuristic fallback.** Resolution: the named preference if present, else the first installed model, else nil. When nil the engine declines fake-3D (falls back to the flat 2D player via `onPlaybackError`) rather than warping heuristically — `CoreMLDepthProvider.hasAvailableModel()` gates this in `Pseudo3DStereoEngine.load`. Documents is inbox-only (not searched at load), so the store is the single source of truth for what can load. A `depthStabilize` compute kernel (spatial gaussian band-limit + motion-adaptive temporal EMA, ping-pong `r16Float`) removes coarse-grid aliasing/ripple. Absent a model, the pipeline uses the heuristic warp.
